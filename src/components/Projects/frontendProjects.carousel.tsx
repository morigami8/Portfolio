import { CarouselProject } from './interfaces';
import { SiNestjs, SiPostgresql, SiReact, SiTypescript } from 'react-icons/si';
export const FrontendProjects: CarouselProject = {
  title: 'Frontend',
  list: [
    {
      title: 'Coming soon!',
      description: 'Stock and Crypto Visualizer',
      stackIcons: [
        { name: 'NestJs', label: <SiNestjs /> },
        { name: 'Postgres', label: <SiPostgresql /> },
        { name: 'TypeScript', label: <SiTypescript /> },
        { name: 'React', label: <SiReact /> },
      ],
    },
  ],
};
